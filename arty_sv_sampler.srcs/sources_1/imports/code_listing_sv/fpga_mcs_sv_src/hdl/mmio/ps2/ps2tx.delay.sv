module ps2tx
   ...
   // fsm state type 
   typedef enum {idle, waitr, rts, prepare, start, data, stop} state_type;
   ...
   begin
      ...
      case (state_reg)
         idle: begin
            ...
            if (wr_ps2) begin
               b_next = {par, din};
               c_next = 14'h2710; // 10000 in hex equivalent to 100us
               state_next = waitr;
            end
         end
         waitr:
         ...
         rts: begin  // request to send
            ...
            tri_c = 1'b1;
            c_next = c_reg - 1;
            d_next = 13'h1F40;  //8000 in hex equivalent to 80us
            if (c_reg==0)
            begin
                ps2d_out = 1'b0;
                tri_d = 1'b1;
         ...
         prepare: begin
            d_next = d_reg - 1;
            if (d_reg==13'hFA0) //Check if count is at 40us remaining
            begin
                tri_c = 1'b0;     //Release clock after data has been low (for 40us, chosen arbitrarily)
                tri_d = 1'b0;     //Also release data simultaneously
                state_next = start;
            end
         end
         start: begin // assert start bit
            d_next = d_reg - 1;
            if (d_reg==0)  //wait until count goes zero (40us total)
            begin
               ps2d_out = 1'b0;
               tri_d = 1'b1;
            end
